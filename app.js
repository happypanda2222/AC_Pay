...
async function loadAirportLookup(){
  ...
  airportLookupPromise = fetch(IATA_LOOKUP_URL, { cache: 'force-cache', mode: 'cors' })
    .then(resp => { if (!resp.ok) throw new Error(`Airport directory error (${resp.status})`); return resp.json(); })
    .then(list => {
      const map = {};
      (list || []).forEach(entry => {
...
async function fetchWeatherForAirport(icao){
  const [metarRes, tafRes] = await Promise.all([
    fetch(`${WEATHER_API_ROOT}/metar?format=json&ids=${icao}`, { cache:'no-store', mode:'cors' }),
    fetch(`${WEATHER_API_ROOT}/taf?format=json&ids=${icao}`, { cache:'no-store', mode:'cors' })
  ]);
  if (!metarRes.ok && !tafRes.ok){
    throw new Error(`Weather API unavailable (${metarRes.status || tafRes.status}) for ${icao}`);
  }
...
async function runWeatherWorkflow({ depId, arrId, depHrsId, arrHrsId, outId, rawId }){
  ...
  try {
    ...
    console.error(err);
    if (outEl) outEl.innerHTML = `<div class="wx-error">${escapeHtml(err.message || 'Weather lookup failed')}. Please check your network connection and try again.</div>`;
    if (rawEl) rawEl.innerHTML = '';
  }
}
